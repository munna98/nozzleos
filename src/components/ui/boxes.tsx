...
const BoxCell = React.memo(({ showPlus }: { showPlus: boolean }) => (
  <motion.div
    className="relative h-8 w-16 border-r border-t border-border/50"
    whileHover={{
  ...
  >
    {showPlus && (
      <svg
        className="pointer-events-none absolute -left-[22px] -top-[14px] h-6 w-10 text-border/50"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.5"
        viewBox="0 0 24 24"
...
const BoxRow = React.memo(({ rowIndex, cols }: { rowIndex: number; cols: number }) => (
  <div className="relative h-8 w-16 border-l border-border/50">

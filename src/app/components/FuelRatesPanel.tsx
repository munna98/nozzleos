...
import { PencilEdit01Icon, Loading02Icon, Tick02Icon, FuelStationIcon, Cancel01Icon } from "@hugeicons/core-free-icons"
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export function FuelRatesPanel() {
    ...
    const [syncNozzles, setSyncNozzles] = useState(false)
    ...
    const handleSave = (id: number) => {
        ...
        if (!isNaN(price) && price >= 0) {
            updateMutation.mutate({ id, price, syncNozzlePrices: syncNozzles })
        }
    }
                                    ...
                                    {editingId === fuel.id ? (
                                        <div className="flex items-center gap-1 px-2 w-full animate-in fade-in zoom-in-95 duration-200">
                                            ...
                                            <div className="flex gap-0.5">
                                                <Button size="icon" variant="ghost" className="h-7 w-7 text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => setShowConfirm(true)}>
            ...
            <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Confirm updating the fuel price to ₹{parseFloat(editPrice).toFixed(2)}
                        </DialogDescription>
                    </DialogHeader>
                    <div className="flex items-center gap-3 py-4">
                        <Checkbox
                            id="syncNozzles"
                            checked={syncNozzles}
                            onCheckedChange={(checked) => setSyncNozzles(checked === true)}
                        />
                        <label htmlFor="syncNozzles" className="text-sm font-medium cursor-pointer">
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={() => handleSave(editingId!)} disabled={updateMutation.isPending}>
